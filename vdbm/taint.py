...
def sink(func):
    ...
    def wrapper(*args, **kwargs):
        if any(isinstance(e, tstr) for e in list(args) + list(kwargs.values())):
            raise Exception("tainted")
        return func(*args, **kwargs)
    return wrapper

def mark_sinks(module):
    for (module, name, obj) in mark(module): setattr(module, name, sink(obj))
...
class ProxyIter(collections.Iterator):
    ...
    def __next__(self): return tstr(self.i.__next__())

    def __hasattr__(self, name): return hasattr(self.i, name)

    def __getattr__(self, name):
        func = getattr(self.i,name)
        return source(func) if func else None
...
def mark_sources(module):
    for (module, name, obj) in mark(module): setattr(module, name, source(obj))

# Propagation

def make_strtuple_wrapper(fun):
    return lambda *a, **kw: tuple(tstr(l) for l in fun(*a, **kw))

def make_strlst_wrapper(fun):
    return lambda *a, **kw: [tstr(l) for l in fun(*a, **kw)]

def make_str_wrapper(fun):
    return lambda *a, **kw: tstr(fun(*a, **kw))
